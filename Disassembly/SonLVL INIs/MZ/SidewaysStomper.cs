...
namespace S12005ObjectDefinitions.MZ
{
	class SidewaysStomper : ObjectDefinition
	{
		...
		{
			byte[] blocks = ObjectHelper.OpenArtFile("../art/nemesis/mzmetal.bin", CompressionType.Nemesis);
			byte[] padding = new byte[0x4360-blocks.Length];
			byte[] spikes = ObjectHelper.OpenArtFile("../art/nemesis/spikes.bin", CompressionType.Nemesis);
			...
			for (int i = 0; i < labels.Length; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(artfile, "../mappings/sprite/obj45.asm", labels[i], 0));
		}
